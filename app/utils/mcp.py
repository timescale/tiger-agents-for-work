...
async def get_user_metadata(user_id: str) -> SlackUserResult | None:
    try:
        result = await slack_mcp_server().direct_call_tool("getUsers", { "keyword": user_id, "includeTimezone": True })
...
async def get_memories(user_id: str) -> list[Memory] | None:
    try:
        result = await memory_mcp_server().direct_call_tool("getMemories", {"key": f"{AGENT_NAME}:{user_id}"}, None)
