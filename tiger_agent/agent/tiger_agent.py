    ...
    async def create_and_send_response(
        ...
        if isinstance(event, SalesforceBaseEvent):
            ...
            original_message = await post_response(
                ...
                thread_ts=None,
                text=f"*New Case <{create_case_url(event.case)}|{event.case.CaseNumber}> - {event.case.Subject}{f', assigned to <@{response.output.case_owner_slack_user_id}>' if response.output.case_owner_slack_user_id else ''}*\n{response.output.short_description_of_case}\Details in :thread:",
            )
