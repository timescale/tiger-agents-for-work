...
from tiger_agent.slack.slash_commands.handlers.admins.remove import handle
...
class TestAdminsRemoveHandler:
    ...
    async def test_returns_confirmation_with_username_on_success(self, ctx):
        ...
        assert result == "Removed admin <nathan>"

    async def test_calls_delete_admin_user_sql_function(self, ctx):
        await handle(ctx, ["<@U123|nathan>"])
        sql, _ = ctx.hctx.pool._cursor.execute.call_args.args
        assert "delete_admin_user" in sql
