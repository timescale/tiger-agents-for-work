...
from tiger_agent.slack.slash_commands.handlers.ignored.add import handle
...
class TestIgnoredAddHandler:
    ...
    async def test_returns_confirmation_with_username_on_success(self, ctx):
        ...
        assert result == "Ignored <nathan>"

    async def test_calls_insert_ignored_user_sql_function(self, ctx):
        await handle(ctx, ["<@U123|nathan>"])
        sql, _ = ctx.hctx.pool._cursor.execute.call_args.args
        assert "insert_ignored_user" in sql
